...
/*
 * PWM Controller Driver for sunxi platforms (D1, T113-S3, R329 and A523)
 *
 * This is the mainline pwm-sun20i.c driver (Aleksandr Shubin), ported into the
 * Trimui Smart Pro S A523 mainline display port and adapted to the modern
 * pwm_chip allocation API (devm_pwmchip_alloc/pwmchip_get_drvdata) used by
 * Linux v7.1-rc7. An "allwinner,sun55i-a523-pwm" compatible is added: the A523
 * (sun55iw3) "PWM v201" block is register-compatible with the D1 "PWM v2" IP
 * (same CLK_CFG/CLK_GATE/ENABLE/CTL/PERIOD layout; verified against the vendor
 * BSP pwm-sunxi.h register map and the A523 UM PWM controller 0 @ 0x02000c00).
 *
 * Limitations:
 * - When the parameters change, the currently running period will not be
 *   completed and the new settings run immediately.
 * - It outputs a HIGH-Z state when the PWM channel is disabled.
 *
 ...
 * without using a loop. See the upstream commit message for the derivation.
 */
...
struct sun20i_pwm_chip {
	...
	void __iomem *base;
	/* Mutex to protect the shared CLK_GATE/ENABLE registers */
	struct mutex mutex;
};
